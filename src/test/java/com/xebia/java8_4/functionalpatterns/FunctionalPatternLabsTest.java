...
public class FunctionalPatternLabsTest
{
	...
	{
		final List<Person> expected = new ProgrammersFilter().filterPersonsImperativeImpl(csvUrl);
		final List<Person> result = new ProgrammersFilter().filterPersonsFunctionalImpl(csvUrl);
		assertThat(result, equalTo(expected));
	}

	/**
	 * Instruction: Implement
	 * the higher order function {@link FunctionalPersonFilter#filterPersons(URL, Predicate)}. In this functional implementation
	 * the
	...
	{
		final List<Person> expected = new ProgrammersFilter().filterPersonsImperativeImpl(csvUrl);
		final List<Person> result = FunctionalPersonFilter.filterPersons(csvUrl, p -> p.getRole() == Role.PROGRAMMER);
		assertThat(result, equalTo(expected));
	}
