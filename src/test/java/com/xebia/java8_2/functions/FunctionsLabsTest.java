...
public class FunctionsLabsTest
{
	...
	{
		final Predicate<Person> combinedFemaleOrMinors = selectFemaleOrMinorsWithCombinedPredicates();
	...
	@Test
	public void shouldSelectMinorMalePersonWithCombinedPredicates()
	{
		final Predicate<Person> combinedPredicate = FunctionsLab.selectMinorMalePersonWithCombinedPredicates();
	...
	{
		final Consumer<Person> consumePerson = logWithConsumer();
	...
	{
		final Consumer<Person> consumePerson = logWithMethodReference();
		consumePerson.accept(p1);
		assertTrue(SimpleLogger.isLogged(p1.toString()));
	}
	...
	{
		final Persons sorted = persons.sort(sortByName());
	...
	{
		final Supplier<String> supplier = nameOfPersonWithLambdaSupplier(p1);
	...
	{
		final Supplier<String> supplier = nameOfPersonWithMethodReferenceSupplier(p1);
		assertEquals(p1.getName(), supplier.get());
	}
